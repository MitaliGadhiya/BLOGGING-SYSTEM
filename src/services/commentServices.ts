...
export class CommentServices {
  ...
  async updateComment(
    ...
    const result1 = await BlogpostModel.findOne({
      _id: blogpostID,
      isdeleted: false
    })
    if (!result1) {
  ...
  async deleteComment(
    ...
    const blogresult = await BlogpostModel.findOne({
      _id: blogpostID,
      isdeleted: false
    })
    if (!blogresult) {
    ...
    const commentresult = await CommentModel.findOne({
      _id: commentID,
      blogpostID: blogpostID,
      isdeleted: false
    })
